...
| Нет образа | `task runtime:image:cli` или pull GHCR — [Образы](Ru-Reference-Images) |
| Proxy не достучится до gateway | Выберите gateway; sidecar → `host.whaleshell.internal` |
| `CONNECT 403` / policy deny | Узкий allow через `whaleshell rule approve` / policy set — [POLICY_LOCAL](../../../docs/POLICY_LOCAL.md) |
